	...
	NDefines.NMilitary.LAND_AIR_COMBAT_STR_DAMAGE_MODIFIER = 0.04    -- （更改）空中对地强度伤害修正
	NDefines.NMilitary.LAND_AIR_COMBAT_ORG_DAMAGE_MODIFIER = 0.14    -- （更改）空中对地组织度伤害修正
	...
	NDefines.NMilitary.ANTI_AIR_TARGETTING_TO_CHANCE = 0.05 -- 0.2（更改）防空瞄准值转换为命中率的平衡系数
	NDefines.NMilitary.ANTI_AIR_ATTACK_TO_AMOUNT = 0.001 -- 0.01（更改）防空瞄准值转换为命中率的平衡系数
	...
	NDefines.NAir.BASE_STRATEGIC_BOMBING_HIT_PLANE_CHANCE = 0.01 -- 0.5（更改）战略轰炸命中机场飞机的概率
	...
	NDefines.NAir.AIR_WING_XP_TRAINING_MISSION_ACCIDENT_FACTOR = 0.002 -- （更改）训练任务导致事故概率的加成系数
	NDefines.NAir.AIR_WING_XP_TRAINING_MISSION_GAIN_DAILY = 10 -- （更改）执行训练任务时每日获得的经验值
	...
	NDefines.NNavy.COMBAT_DAMAGE_TO_STR_FACTOR = 1.2 -- 1.6	更改（伤害造成的HP损失）
	NDefines.NNavy.COMBAT_DAMAGE_TO_ORG_FACTOR = 2.0 -- 1.9	更改（伤害造成的组织度损失）
	...
	NDefines.NNavy.ANTI_AIR_TARGETTING_TO_CHANCE = 0.05 -- 0.2（更改）防空瞄准值转换为命中率的平衡系数
	NDefines.NNavy.ANTI_AIR_ATTACK_TO_AMOUNT = 0.01 -- 0.01（更改）防空瞄准值转换为命中率的平衡系数
	...
	NDefines.NNavy.GUN_HIT_PROFILES = { -- hit profiles for guns, if target ih profile is lower the gun will have lower accuracy
		100.0,	-- big guns
		2000.0,	-- torpedos  #anti ship guided weapons
		100.0,	-- small guns
	}
	NDefines.NNavy.BASE_GUN_COOLDOWNS = { -- number of hours for a gun to be ready after shooting
		1.0,	-- big guns
		2.0,	-- torpedos #anti ship guided weapons
		1.0,	-- small guns
	}

	NDefines.NNavy.DEPTH_CHARGES_HIT_CHANCE_MULT = 3 -- （更改）深水炸弹命中概率乘数
	NDefines.NNavy.DEPTH_CHARGES_DAMAGE_MULT = 2 	-- multiplies damage of depth charges
	NDefines.NNavy.DEPTH_CHARGES_HIT_PROFILE = 4.0	-- （更改）深水炸弹的命中剖面
	...
	NDefines.NNavy.TRAINING_ACCIDENT_CRITICAL_HIT_CHANCES = 0.001 -- 0.3（更改）训练事故触发致命伤害的概率
	...
	NDefines.NNavy.COMBAT_BASE_CRITICAL_CHANCE = 0.3 --0.05	更改（基础暴击倍率）
	...
	NDefines.NNavy.SCREENS_TO_CAPITAL_RATIO = 3				-- （更改）海战中创建火力交换组时的主力舰与屏卫舰比例

	NDefines.NNavy.BASE_POSITIONING = 0.4 -- 1.0	-- 更改（阵型效能基础值）
	NDefines.NNavy.RELATIVE_SURFACE_DETECTION_TO_POSITIONING_FACTOR = 0.01 -- 0.01
	NDefines.NNavy.MAX_POSITIONING_BONUS_FROM_SURFACE_DETECTION = 0.75 -- 0.0	更改（探测优势带来的阵型效能加成上限）
	NDefines.NNavy.HIGHER_SHIP_RATIO_POSITIONING_PENALTY_FACTOR = 0.0 -- 0.25	更改（舰船数量优势惩罚系数：当一方舰船数量每超过对方100%时，该方阵型效能将受到此系数值的惩罚）
	NDefines.NNavy.MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO = 0.0 -- 0.5	更改（舰船数量优势带来的最大阵型效能惩罚）
	NDefines.NNavy.MIN_SHIPS_FOR_HIGHER_SHIP_RATIO_PENALTY = 5000 -- 0	更改（触发大舰队惩罚的最小舰船数量阈值（舰队规模达到该值后才应用惩罚））
	NDefines.NNavy.HIGHER_CARRIER_RATIO_POSITIONING_PENALTY_FACTOR = 0.0 -- 0.2	更改（航母空中力量优势方的阵型效能惩罚系数）
	NDefines.NNavy.MAX_CARRIER_RATIO_POSITIONING_PENALTY_FACTOR = 0.0 -- 0.2 更改（航母空中力量优势带来的最大效能惩罚）
	NDefines.NNavy.POSITIONING_PENALTY_FOR_SHIPS_JOINED_COMBAT_AFTER_IT_STARTS = 0.01 -- 0.05 更改（战斗开始后每艘新加入舰船的阵型效能惩罚）
	NDefines.NNavy.EXPERIENCE_FACTOR_NON_CARRIER_GAIN = 1 --（更改） 非航母舰船战斗经验获取系数
	NDefines.NNavy.EXPERIENCE_FACTOR_CARRIER_GAIN = 1.25 -- （更改）航母战斗经验获取系数
	...
	NDefines.NNavy.SUBMARINE_REVEALED_TIMEOUT = 96		-- Amount of in-game-hours that makes the submarine visible if it is on the defender side.
	...
	NDefines.NNavy.SUBMARINE_BASE_TORPEDO_REVEAL_CHANCE = 0.8		-- Chance of a submarine being revealed when it fires. 1.0 is 100%. this chance is then multiplied with modifier created by comparing firer's visibiility and target's detection
	...
	NDefines.NNavy.NAVY_PIERCING_THRESHOLDS = {	-- （更改）穿甲阈值（我方穿甲/敌方装甲≥此值时触发对应伤害比例）
		1.50,
		1.00,
		0.75,
		0.50,
	...
	NDefines.NNavy.NAVY_PIERCING_THRESHOLDS = {	-- （更改）穿甲暴击阈值（格式需与上方一致）
		1.50,
		1.00,
		0.90,
		0.80,
	...
	NDefines.NNavy.NAVY_PIERCING_THRESHOLDS = {	-- （更改）穿甲伤害比例（对应上方阈值）
		1.50,
		1.25,
		1.00,
		0.90,
	...
	NDefines.NNavy.MIN_SPOTTING_PROGRESS = 0.05	-- （更改）每小时最低发现进度（百分比）
	--NDefines.NNavy.SUPREMACY_PER_SHIP_PER_MANPOWER = 1 -- （更改）单舰制海权计算中人力系数
	--NDefines.NNavy.SUPREMACY_PER_SHIP_BASE = 0.3  （更改）单舰制海权基础值
	...
	NDefines.NAI.MAX_MISSION_PER_TASKFORCE = {  -- (更改)每任务部队的最大任务区域比例
				0, -- 待命
				2, -- 巡逻
				10, -- 打击舰队
				1.5, -- 运输船袭击
	...
	NDefines.NAI.HIGH_PRIO_NAVAL_MISSION_SCORES = {  -- (更改)高优先级任务的评分
				0, -- 待命
				100000, -- 巡逻
				1000, -- 打击舰队
				1500, -- 运输船袭击
	...
	NDefines.NAI.CARRIER_TASKFORCE_MAX_CARRIER_COUNT = 20 		-- (更改)航母特遣舰队的最佳航母数量
	NDefines.NAI.CAPITAL_TASKFORCE_MAX_CAPITAL_COUNT = 60 		-- (更改)主力舰特遣舰队的最佳主力舰数量
	NDefines.NAI.SCREEN_TASKFORCE_MAX_SHIP_COUNT = 180			-- (更改)屏卫舰特遣舰队的最佳屏卫舰数量
	NDefines.NAI.SUB_TASKFORCE_MAX_SHIP_COUNT = 30				-- (更改)潜艇特遣舰队的最佳潜艇数量
	NDefines.NAI.MAX_UNIT_RATIO_FOR_INVASIONS = 0.8            -- (更改)最大登录比例
	NDefines.NAI.MIN_UNIT_RATIO_FOR_INVASIONS = 0.3            -- (更改)最小登录比例
	NDefines.NAI.MAX_INVASION_SIZE = 96                             -- (更改)登陆部队的最大规模。
	NDefines.NAI.MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MIN = 0.30 -- (更改)护航任务的最小屏卫舰分配比例。
	NDefines.NAI.MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MAX = 0.90 -- (更改)护航任务的最大屏卫舰分配比例。
	NDefines.NAI.MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MIN_CONVOY_THREAT = 50 -- (更改)护航威胁增加分配比例的起始值。
	NDefines.NAI.MAX_SCREEN_TASKFORCES_FOR_CONVOY_DEFENSE_MAX_CONVOY_THREAT = 200 -- (更改)护航威胁达到此值时分配比例达最大值。
	NDefines.NAI.MAX_CAPITAL_FORCES_FOR_INVASION_SUPPORT = 1 -- （更改）登陆支援任务的主力舰分配上限。
	NDefines.NAI.MAX_SCREEN_FORCES_FOR_INVASION_SUPPORT = 0.2 -- （更改）登陆支援任务的屏卫舰分配上限。
	NDefines.NAI.PORT_STRIKE_HISTORY_VALUE_TO_DISABLE_REPAIRS = 100		-- （更改）港口打击历史值超过此阈值时停用维修
	NDefines.NAI.PORT_STRIKE_HISTORY_VALUE_TO_REENABLE_REPAIRS = 5     -- （更改）港口打击历史值低于此阈值时重新启用维修
	NDefines.NAI.NAVAL_SUPREMACY_WEIGHT_PER_DIVISION_ON_INVASION_ORDER = 40 -- (更改)存在登陆计划时，区域制海权需求增加。
	NDefines.NAI.REGION_THREAT_PER_SUNK_CONVOY = 1					-- (更改)每艘运输船被击沉产生的区域威胁值。
	NDefines.NAI.REGION_THREAT_LEVEL_TO_AVOID_REGION = 950					-- (更改)区域威胁值达到此值时，AI避开该区域。
	NDefines.NAI.REGION_THREAT_LEVEL_TO_BLOCK_REGION = 9500			-- (更改)区域威胁值达到此值时，AI完全封锁该区域。
